...
def add_trees(t1, t2):
    ...
    i=0
    branch=[]
    while len(t1.branches) > i and len(t2.branches) >i:
        branch+=[add_trees(t1.branches[i],t2.branches[i])]
        i+=1
    branch+=t1.branches[i:]
    branch+=t2.branches[i:]
...
def match_url(text):
    ...
    path = r'(?:\/\w+)+(?:\.?\w+)?'
    anchor = r'/#.*'
    print("DEBUG",re.findall(rf"^(?:{scheme})?{domain}(?:{path})?(?:{anchor})?$", text))
