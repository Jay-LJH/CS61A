    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self):
                ...
                place.ant = self
            elif place.ant.is_container and (place.ant.ant_contained or self.is_container):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, lower_bound=0, upper_bound=1e6):
        ...
        count = 0
        while True:
            if p.bees and not p.is_hive and lower_bound <= count <= upper_bound:
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        return max(0,2-0.25*distance-self.insects_shot*0.0625)
        # END Problem Optional 2

    def action(self, gamestate):
        insects_and_distances = self.insects_in_front()
        for insect, distance in insects_and_distances:
            damage = self.calculate_damage(distance)
            print("DEBUG", insect.name, damage)
