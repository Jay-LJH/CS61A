...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    d1=  distance(make_city('',lat,lon),city_a)
    d2 = distance(make_city('', lat, lon), city_b)
    if d1<=d2:
...
def sprout_leaves(t, leaves):
    ...
    """
    "*** YOUR CODE HERE ***"
    bran=[tree(i) for i in leaves]
    def func(t):
        if is_leaf(t):
            return tree(label(t), bran)
        dic=[func(i) for i in branches(t)]
        return tree(label(t),dic)
    return func(t)
...
def add_trees(t1, t2):
    ...
    bran=[add_trees(b1[i],b2[i]) for i in range(length)]
    if(len(b1)>length):
        bran+=b1[length:]
    if(len(b2)>length):
        bran+=b2[length:]
