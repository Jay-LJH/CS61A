...
def invert_short(x, limit):
    ...
    100
    """
    return limited(x, 1/x if x!=0 else limit, limit)
...
def change_short(x, y, limit):
    ...
    100
    """
    return limited(x, abs(y - x) / x if x!=0 else limit, limit)
...
def hailstone(n):
    ...
    if n == 1:
        return 1;
    return hailstone(n/2)+1 if n % 2 ==0 else hailstone(3*n+1)+1

"*** YOUR CODE HERE ***"
quine = r"var = 'print('var = ' + repr(var) + ';eval(var)')';eval(var)"
