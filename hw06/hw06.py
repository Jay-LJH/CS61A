class VendingMachine:
    ...
    def vend(self):
        ...
        elif self.fund == self.price and self.stock > 0:
            self.stock -= 1
            self.fund = 0
    ...
    def restock(self, num):
        self.stock += num
        return fr'Current {self.name} stock: {self.stock}'
...
def two_list(vals, amounts):
    ...
    for i in range(len(amounts)):
        if amounts[i]>0:
            amounts[i]-=1
            print("DEBUG",amounts)
            return Link(vals[i],two_list(vals,amounts))
    return Link.empty
...
class VirFib():
    ...
    def __init__(self, value=0,before=0):
        self.value = value
        self.before=before
    ...
    def next(self):
        ...
        if self.value==0:
            ...
            return VirFib(self.value+self.before,self.value)
...
def bst_max(t):
    if t.branches:
        return max(t.label,max(list(map(bst_max,t.branches))))
    return t.label
def bst_min(t):
    if t.branches:
        return min(min(list(map(bst_min,t.branches))),t.label)
    return t.label
class Link:
    """A linked list.
